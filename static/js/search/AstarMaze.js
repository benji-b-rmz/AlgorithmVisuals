...
function Astar(start, goal){
    ...
    while (openSet.length > 0){
        ...
        for(var i = 0; i < neighbors.length; i++){
            ...
            fScore[neighbors[i].col][ neighbors[i].row] = gScore[neighbors[i].col][ neighbors[i].row]
                + manhattanDistance(neighbors[i], goal);
...
function animate(){
    ...
    if (traversing) {
        ...
        search = Astar(gridCells[0][0], gridCells[COLS-1][ROWS-1]);
        console.log(search);
